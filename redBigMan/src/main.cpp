...
void usercontrol(void) {
  ...
  bool load =true,eat=false,thro=false;
  int standposition=0;
  while (1) {
    ...
    standposition=0.5*leftStandMotor.position(degrees)+0.5*rightStandMotor.position(degrees);

    Brain.Screen.clearLine();
    Brain.Screen.print(standposition);
    if((standposition>10) || (standposition<500)) load =true;
    if(load)
    {
